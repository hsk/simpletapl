...
:- run([eval(app(abs(x,nat, var(x)), zero)) ] ).
:- run([eval(app(abs(x,nat, var(x)), succ(zero))) ] ).
:- run([eval(app(abs(x,nat, succ(var(x))), zero)) ] ).
:- run([eval(app(abs(x,nat, succ(succ(var(x)))), succ(zero))) ] ).
:- run([bind('T', bVar(arr(nat,nat)))]).
:- run([bind('T', bVar(arr(nat,nat))),
        eval(abs(f,arr(nat,nat), abs(x,nat, app(var(f), app(var(f),var(x))))))]).

:- run([bind('T', bTAbb(arr(nat,nat),none)),
        eval(abs(f,tvar('T'), var(f))) ]).
:- run([bind('T', bTAbb(arr(nat,nat),none)),
        eval(abs(f,tvar('T'), app(var(f),zero))) ]).

:- run([bind('T', bTAbb(arr(nat,nat),none)),
        eval(abs(f,tvar('T'), abs(x,nat, app(var(f), app(var(f),var(x))))))]).

:- run([eval(tabs('X', kStar,abs(x,tvar('X'), var(x))))]). 
:- run([eval(tapp(tabs('X', kStar,abs(x,tvar('X'), var(x))), all('X',app(tvar('X',tvar('X'))))))]).

:-run([
    bind('Pair',bTAbb(abs('X',kStar,abs('Y',kStar,all('R',kStar,arr(arr(tvar('X'),arr(tvar('Y'),tvar('R'))),tvar('R'))))),none)),
    bind(pair,bMAbb(tabs('X',kStar,tabs('Y',kStar,abs(x,tvar('X'),abs(y,tvar('Y'),tabs('R',kStar,abs(p,arr(tvar('X'),arr(tvar('Y'),tvar('R'))),app(app(var(p),var(x)),var(y)))))))),none)),
    bind(fst,bMAbb(tabs('X',kStar,tabs('Y',kStar,abs(p,app(app(tvar('Pair'),tvar('X')),tvar('Y')), app(tapp(var(p),tvar('X')), abs(x,tvar('X'),abs(y,tvar('Y'),var(x))) ) ))),none)),
    bind(snd,bMAbb(tabs('X',kStar,tabs('Y',kStar,abs(p,app(app(tvar('Pair'),tvar('X')),tvar('Y')), app(tapp(var(p),tvar('Y')), abs(x,tvar('X'),abs(y,tvar('Y'),var(y))) ) ))),none)),
