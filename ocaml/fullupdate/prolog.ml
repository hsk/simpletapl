...
let rec t = function
  ...
  | TRecord(fields) ->
      let pf (li,(vari,ti)) =
        li ^ ":(" ^ (if vari=Invariant then "invariant" else "covariant") ^","^t ti^")"
      in
...
let rec m = function
  ...
  | MUpdate(m1, l, m2) -> Printf.sprintf "update(%s,%s,%s)" (m m1) l (m m2)
  ...
  | MRecord(fields) ->
      let pf (li,(vari,mi)) =
        li ^ "=" ^ (if vari=Invariant then "(invariant," else "(covariant,") ^ m mi^")"
      in
      "record([" ^ String.concat ", " (List.map pf fields) ^ "])"
