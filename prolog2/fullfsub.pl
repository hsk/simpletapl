...
typeof(G,pack(T1,M2,T),T) :- simplify(G,T,some(Y,TBound,T2)),subtype(G,T1,TBound),typeof(G,M2,S2),tsubst(Y,T1,T2,T2_),subtype(G,S2,T2_).
...
run(someBind(TX,X,M),G,[X-bMAbb(T12,some(TBody)),TX-bTVar(TBound)|G]) :-
  typeof(G,M,T),simplify(G,T,some(_,TBound,TBody)),eval(G,M,pack(_,T12,_)),writeln(TX),write(X),write(' : '),writeln(TBody).
run(someBind(TX,X,M),G,[X-bVar(TBody),TX-bTVar(TBound)|G]) :-
  typeof(G,M,T),simplify(G,T,some(_,TBound,TBody)),writeln(TX),write(X),write(' : '),writeln(TBody).
