...
t(T) :- T = bool
      ...
      ; T = record(Tf)       , maplist([X:(I,T1)]>>(l(X),ι(I),t(T1)),Tf)
      ; T = all(X,T1,T2)     , atom(X),t(T1),t(T2)
      ; T = some(X,T1,T2)    , atom(X),t(T1),t(T2)
...
m(M) :- M = true
      ...
      ; M = record(Tf)       , maplist([X=(I,M1)]>>(l(X),ι(I),m(M1)), Mf)
      ; M = update(M1,L,M2)  , m(M1),l(L),m(M2)
      ; M = proj(M1,L)       , m(M1),l(L)
      ; M = pack(T1,M1,T2)   , t(T1),m(M1),t(T2)
      ; M = unpack(TX,X,M1,M2), atom(TX),atom(X),m(M1),m(M2)
...
teq2(Γ,record(Sf),record(Tf))        :- length(Sf,Len),length(Tf,Len),
                                          maplist([L:(TVar,T)]>>(member(L:(TVar,S),Sf),teq(Γ,S,T)), Tf).
...
subtype2(Γ,record(SF),record(TF))        :- maplist([L:(Vart,T)]>>(
                                                member(L:(Vars,S),SF),
...
subtype2(Γ,all(TX,S1,S2),all(_,T1,T2))   :- subtype(Γ,S1,T1),subtype(Γ,T1,S1),subtype([TX-bTVar(T1)|Γ],S2,T2).
subtype2(Γ,some(TX,S1,S2),some(_,T1,T2)) :- subtype(Γ,S1,T1),subtype(Γ,T1,S1),subtype([TX-bTVar(T1)|Γ],S2,T2).

join(Γ,S,T,T)                                          :- subtype(Γ,S,T).
join(Γ,S,T,S)                                          :- subtype(Γ,T,S).
join(Γ,S,T,R)                                          :- simplify(Γ,S,S_),simplify(Γ,T,T_),join2(Γ,S_,T_,R).
join2(Γ,record(SF),record(TF),record(UF_))          :- include([L:_]>>member(L:_,TF),SF,UF),
                                                          maplist([L:(SVar,S),L:(Var,T_)]>>(
                                                            member(L:(TVar,T),TF),
                                                            (SVar = TVar,Var = SVar;Var = invariant),
                                                            join(Γ,S,T,T_)),UF,UF_).
join2(Γ,all(TX,S1,S2),all(_,T1,T2),all(TX,S1,T2_))  :- subtype(Γ,S1,T1),subtype(Γ,T1,S1),
...
meet2(Γ,record(SF),record(TF),record(UF_))         :- maplist([L:(SVar,S),L:(Var,T_)]>>(
                                                            member(L:(TVar,T),TF),
                                                            (SVar = TVar,Var = SVar;Var = covariant),
                                                            meet(Γ,S,T,T_);T_ = S),SF,SF_),
                                                         include([L:_]>>(\+member(L:_,SF)),TF,TF_),
                                                         append(SF_,TF_,UF_).
meet2(Γ,all(TX,S1,S2),all(_,T1,T2),all(TX,S1,T2_)) :- subtype(Γ,S1,T1),subtype(Γ,T1,S1),
...
typeof(Γ,true,bool).
typeof(Γ,false,bool).
typeof(Γ,if(M1,M2,M3),T)                :- typeof(Γ,M1,T1),subtype(Γ,T1,bool),
                                            typeof(Γ,M2,T2),typeof(Γ,M3,T3),join(Γ,T2,T3,T).
typeof(Γ,zero,nat).
...
typeof(Γ,timesfloat(M1,M2),float)      :- typeof(Γ,M1,T1),subtype(Γ,T1,float),
                                            typeof(Γ,M2,T2),subtype(Γ,T2,float).
typeof(Γ,string(_),string).
typeof(Γ,var(X),T)                      :- !,gett(Γ,X,T).
typeof(Γ,fn(X,T1,M2),arr(T1,T2_))     :- kindof(Γ,T1,*),typeof([X-bVar(T1)|Γ],M2,T2_),!.
typeof(Γ,app(M1,M2),T12)                :- typeof(Γ,M1,T1),lcst(Γ,T1,arr(T11,T12)),
                                            typeof(Γ,M2,T2),subtype(Γ,T2,T11).
...
typeof(Γ,update(M1, L, M2),T1)          :- typeof(Γ,M1,T1),typeof(Γ,M2,T2),lcst(Γ,T1,record(Tf)),
                                            member(L:(invariant,T),Tf),subtype(Γ,T2,T).
typeof(Γ,pack(T1,M2,T),T)               :- kindof(Γ,T,*),simplify(Γ,T,some(Y,TBound,T2)),
                                            subtype(Γ,T1,TBound),typeof(Γ,M2,S2),
                                            tsubst(Y,T1,T2,T2_),subtype(Γ,S2,T2_).
typeof(Γ,unpack(TX,X,M1,M2),T2)         :- typeof(Γ,M1,T1),lcst(Γ,T1,some(_,TBound,T11)),
                                            typeof([X-bVar(T11),(TX-bTVar(TBound))|Γ],M2,T2).
typeof(Γ,tfn(TX,T1,M2),all(TX,T1,T2)) :- typeof([TX-bTVar(T1)|Γ],M2,T2),!.
typeof(Γ,tapp(M1,T2),T12_)              :- typeof(Γ,M1,T1),lcst(Γ,T1,all(X,T11,T12)),
                                            subtype(Γ,T2,T11),tsubst(X,T2,T12,T12_).
...
run(bind(X,Bind),Γ,[X-Bind_|Γ])                  :- check_bind(Γ,Bind,Bind1),
                                                    evalbinding(Γ,Bind1,Bind_),
                                                    write(X),show_bind(Γ,Bind_,R),writeln(R).
run(someBind(TX,X,M),Γ,[X-B,TX-bTVar(TBound)|Γ]) :- !,typeof(Γ,M,T),
...
check_bind(Γ,bTVar(S),bTVar(S))                 :- kindof(Γ,S,_).
...
check_bind(Γ,bMAbb(M,some(T)),bMAbb(M,some(T))) :- typeof(Γ,M,T1), subtype(Γ,T1,T).
...
:- run([eval(app(fn(r,record([x:(covariant,arr(top,top))]),app(proj(var(r),x),proj(var(r),x))),
                  record([x=(covariant,fn(z,top,var(z))),y=(covariant,fn(z,top,var(z)))])))]).
% "hello";
...
% {x=true, y=false};
:- run([eval(record([x=(covariant,true),y=(covariant,false)])) ]).
% {x=true, y=false}.x;
:- run([eval(proj(record([x=(covariant,true),y=(covariant,false)]),x)) ]).
% {true, false};
:- run([eval(record([1=(covariant,true),2=(covariant,false)])) ]).
% {true, false}.1;
:- run([eval(proj(record([1=(covariant,true),2=(covariant,false)]),1)) ]).
...
:- run([eval(pack(nat,record([c=(covariant,zero),f=(covariant,fn(x,nat,succ(var(x))))]),
         some('X',top,record([c:(covariant,var('X')),f:(covariant,arr(var('X'),nat))]))))]).
