...
eval1(G,app(unfold(S),app(fold(T),V1)),V1) :- v(V1).
eval1(G,app(fold(S),M2),app(fold(S), M2_)) :- eval1(G,M2,M2_).
eval1(G,app(unfold(S),M2),app(unfold(S), M2_)) :- eval1(G,M2,M2_).
...
teq2(rec(X,S),rec(_,T)) :- teq([X-bName|G],S,T).
...
typeof(G,fold(S), arr(T_,S)) :- simplify(G,S,rec(X,T)),tsubst(X,S,T,T_).
typeof(G,unfold(S),arr(S,T_)) :- simplify(G,S,rec(X,T)),tsubst(X,S,T,T_).
...
:- run([bind('Counter',bTAbb(rec('P',record([get:nat, inc:arr(unit,var('P'))])))),
  bind(p,bMAbb(let(create,
    fix(
      fn(cr,arr(record([x:nat]),var('Counter')),
        fn(s,record([x:nat]),
          app(fold(var('Counter')),
  ...
  bind(p1,bMAbb(app(proj(app(unfold(var('Counter')),var(p)),inc),unit ),none )),
  eval(proj(app(unfold(var('Counter')),var(p1)),get))
