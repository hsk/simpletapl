...
%eval1(G,M,_) :- \+val(M),writeln(eval1(G,M)),fail.
...
compute(G,X,T) :- val(X),gettabb(G,X,T).
...
teq2(G,bool,bool).
teq2(G,nat,nat).
teq2(G,X,T) :- val(X),gettabb(G,X,S),teq(G,S,T).
teq2(G,S,X) :- val(X),gettabb(G,X,T),teq(G,S,T).
teq2(G,X,X) :- val(X).
...
:- run([
    ...
    eval(a),
    eval(fn(x:bool->x)$a),
    eval(fn(x:bool->fn(x:bool->x)$x)$a),
    eval(fn(x:bool->x)$true),
    eval(fn(x:bool->fn(x:bool->x)$x)$true)
]).

:- run([eval(fn(x:'A'->x))]).
:- run([eval(let(x=true,x))]).
:- run([eval(fn(x:bool->x))]).
:- run([eval(fn(x:(bool->bool)->if(x$false, true, false))$
             fn(x:bool-> if(x,false,true)) )]). 
...
:- run([bind('T', bVar(nat->nat)),
        eval(fn(f:(nat->nat)-> fn(x:nat->f$(f$x))))]).
:- run([bind('T', bTAbb(nat->nat)),
        eval(fn(f:'T'->f)) ]).
:- run([bind('T', bTAbb(nat->nat)),
        eval(fn(f:'T'->f $ 0)) ]).
:- run([bind('T', bTAbb(nat->nat)),
        eval(fn(f:'T'->fn(x:nat->f $ (f $ x)))) ]).
