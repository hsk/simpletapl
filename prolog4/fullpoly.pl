...
Γ /- ({(T1, M2)} as T) : T where simplify(Γ, T, {some(Y), T2}), Γ /- M2 : S2, T2![(Y -> T1)] tsubst T2_, Γ /- S2 = T2_.
Γ /- (let(TX, X) = M1 in M2) : T2 where Γ /- M1 : T1, simplify(Γ, T1, {some(_), T11}), [X - bVar(T11), TX - bTVar | Γ] /- M2 : T2.
...
run({(TX, X)} = M, Γ, [X - bMAbb(T12, some(TBody)), TX - bTVar | Γ]) :- tx(TX), x(X), m(M), Γ /- M : T, simplify(Γ, T, {some(_), TBody}), Γ /- M ==>> {(_, T12)} as _, format('~w\n~w : ~w\n', [TX, X, TBody]).
run({(TX, X)} = M, Γ, [X - bVar(TBody), TX - bTVar | Γ]) :- tx(TX), x(X), m(M), Γ /- M : T, simplify(Γ, T, {some(_), TBody}), format('~w\n~w : ~w\n', [TX, X, TBody]).
