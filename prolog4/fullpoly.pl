...
list(A) ::= [] | [A | list(A)].
t ::= bool | nat | unit | float | string | x | (t -> t) | {list(l : t)} | (some(x) => t) | (all(x) => t).
m ::= true | false | if(m, m, m) | 0 | succ(m) | pred(m) | iszero(m) | unit | floatl | m * m | stringl | x | (fn(x : t) -> m) | m $ m | (let(x) = m in m) | fix(m) | inert(t) | m as t | {list(l = m)} | m # l | m as t | pack(t, m, t) | unpack(x, x, m, m) | (fn(x) => m) | m![t].
n ::= 0 | succ(n).
v ::= true | false | n | unit | floatl | stringl | (fn(x : t) -> m) | {list(l = v)} | pack(t, v, t) | (fn(x) => m).
