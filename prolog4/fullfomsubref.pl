...
w(W) :- member(W, [bool, nat, unit, float, string, top, true, false, 0, error]).
x(X) :- \+ w(X), atom(X).
l(L) :- atom(L) ; integer(L).
k(K) :- K = '*' ; K = kArr(K1, K2), k(K1), k(K2).
t(T) :- T = bool ; T = nat ; T = unit ; T = float ; T = string ; T = top ; T = bot ; T = X, x(X) ; T = (T1 -> T2), t(T1), t(T2) ; T = {Tf}, maplist([X : T1] >> (l(X), t(T1)), Tf) ; T = variant(Tf), maplist([X : T1] >> (x(X), t(T1)), Tf) ; T = ref(T1), t(T1) ; T = source(T1), t(T1) ; T = sink(T1), t(T1) ; T = all(X, T1, T2), x(X), t(T1), t(T2) ; T = some(X, T1, T2), x(X), t(T1), t(T2) ; T = abs(TX, K, T2), x(TX), k(K), t(T2) ; T = T1 $ T2, t(T1), t(T2).
