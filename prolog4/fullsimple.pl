...
Γ /- X ==> M where x(X), member(X - M : _, Γ).
...
Γ /- (let(X) = M1 in M2) : T where Γ /- M1 : T1, [X : T1 | Γ] /- M2 : T.
...
run(X : T = M, Γ, [X - M : T | Γ]) :- x(X), t(T), m(M), Γ /- M : T_, Γ /- T_ = T, Γ /- M ==>> M_, show(Γ, X - M_ : T).
run(X = M, Γ, [X - M_ : T | Γ]) :- x(X), m(M), Γ /- M : T, Γ /- M ==>> M_, show(Γ, X - M_ : T).
