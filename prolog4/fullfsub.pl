...
Γ /- ({(T1, M2)} as T) : T where simplify(Γ, T, {some(Y :: TBound), T2}), Γ /- T1 <: TBound, Γ /- M2 : S2, T2![(Y -> T1)] tsubst T2_, Γ /- S2 <: T2_.
...
run({(TX, X)} = M, Γ, [X - bMAbb(T12, TBody), TX - bTVar(TBound) | Γ]) :- tx(TX), x(X), m(M), Γ /- M : T, simplify(Γ, T, {some(_ :: TBound), TBody}), Γ /- M ==>> {(_, T12)} as _, format('~w\n~w : ~w\n', [TX, X, TBody]).
run({(TX, X)} = M, Γ, [X - bVar(TBody), TX - bTVar(TBound) | Γ]) :- tx(TX), x(X), m(M), Γ /- M : T, simplify(Γ, T, {some(_ :: TBound), TBody}), format('~w\n~w : ~w\n', [TX, X, TBody]).
