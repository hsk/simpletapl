...
join2(Γ, {SF}, {TF}, {UF_})                                 :- include([L : _] >> member(L : _, TF), SF, UF),
...
meet2(Γ, {SF}, {TF}, {UF_})                                 :- maplist([L : S, L : T_] >> (
...
Γ /- pack(T1, M2, T) : T                    where Γ \- T :: '*', simplify(Γ, T, some(Y, TBound, T2)),
                                                  Γ \- T1 <: TBound, Γ /- M2 : S2, tsubst(Y, T1, T2, T2_), Γ \- S2 <: T2_.
...
run(someBind(TX, X, M), Γ, [X - B, TX - bTVar(TBound) | Γ]) :- !, Γ /- M : T, simplify(Γ, T, some(_, TBound, TBody)),
