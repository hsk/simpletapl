...
t(T) :- T = bool ; T = nat ; T = unit ; T = float ; T = string ; T = top ; T = X, x(X) ; T = (T1 -> T2), t(T1), t(T2) ; T = {Tf}, maplist([X : T1] >> (l(X), t(T1)), Tf) ; T = all(X, T1, T2), x(X), t(T1), t(T2) ; T = some(X, T1, T2), x(X), t(T1), t(T2) ; T = abs(TX, K, T2), x(TX), k(K), t(T2) ; T = T1 $ T2, t(T1), t(T2).
m(M) :- M = true ; M = false ; M = if(M1, M2, M3), m(M1), m(M2), m(M3) ; M = 0 ; M = succ(M1), m(M1) ; M = pred(M1), m(M1) ; M = iszero(M1), m(M1) ; M = unit ; M = F, float(F) ; M = M1 * M2, m(M1), m(M2) ; M = X, string(X) ; M = X, x(X) ; M = (fn(X : T1) -> M1), x(X), t(T1), m(M1) ; M = M1 $ M2, m(M1), m(M2) ; M = (let(X) = M1 in M2), x(X), m(M1), m(M2) ; M = fix(M1), m(M1) ; M = inert(T1), t(T1) ; M = M1 as T1, m(M1), t(T1) ; M = {Mf}, maplist([X = M1] >> (l(X), m(M1)), Mf) ; M = M1 # L, m(M1), l(L) ; M = pack(T1, M1, T2), t(T1), m(M1), t(T2) ; M = unpack(TX, X, M1, M2), x(TX), x(X), m(M1), m(M2) ; M = (fn(TX :: T1) => M1), x(TX), t(T1), m(M1) ; M = M1![T1], m(M1), t(T1).
