...
pack(T1, M2, T3)![(J -> M)]      tmsubst pack(T1_, M2_, T3_)     :- T1![(J -> S)]       tsubst T1_, M2![(J -> M)]      tmsubst M2_,
                                                                    T3![(J -> S)]       tsubst T3_.
...
Γ \- {SF} \/ {TF} : {UF_}                                 :- maplist([L : S, L : T_] >> (member(L : T, TF), Γ /- S \/ T : T_ ; T_ = S), SF, SF_),
                                                             include([L : _] >> (\+ member(L : _, SF)), TF, TF_), append(SF_, TF_, UF_).
Γ \- all(TX, S1, S2) \/ all(_, T1, T2) : all(TX, S1, T2_) :- Γ /- S1 <: T1, Γ /- T1 <: S1, [TX - bTVar(T1) | Γ] /- T1 \/ T2 : T2_.
Γ \- (S1 -> S2) \/ (T1 -> T2) : (S_ -> T_)                :- Γ /- S1 /\ T1 : S_, Γ /- S2 \/ T2 : T_.
