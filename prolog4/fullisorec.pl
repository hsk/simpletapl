...
list(A) ::= [] | [A | list(A)].
t ::= bool | nat | unit | float | string | x | (t -> t) | {list(l : t)} | variant(list(x : t)) | rec(t).
m ::= true | false | if(m, m, m) | 0 | succ(m) | pred(m) | iszero(m) | unit | floatl | m * m | stringl | x | (fn(x : t) -> m) | m $ m | (let(x) = m in m) | fix(m) | inert(t) | m as t | {list(l = m)} | m # l | case(m, list(x = (x, m))) | tag(x, m, t) | fold(t) | unfold(t).
n ::= 0 | succ(n).
