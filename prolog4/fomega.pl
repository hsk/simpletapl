...
t(T) :- T = X, x(X) ; T = (T1 -> T2), t(T1), t(T2) ; T = all(X, K, T1), x(X), k(K), t(T1) ; T = abs(TX, K, T2), x(TX), k(K), t(T2) ; T = T1 $ T2, t(T1), t(T2).
m(M) :- M = X, x(X) ; M = (fn(X : T1) -> M1), x(X), t(T1), m(M1) ; M = M1 $ M2, m(M1), m(M2) ; M = (fn(TX :: K) => M2), x(TX), k(K), m(M2) ; M = M1![T2], m(M1), t(T2).
J![(J -> S)] tsubst S :- x(J).
X![(J -> S)] tsubst X :- x(X).
