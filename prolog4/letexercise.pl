...
w(W) :- member(W, [bool, true, false, 0]).
x(X) :- \+ w(X), atom(X).
t(T) :- T = bool ; T = top ; T = (T1 -> T2), t(T1), t(T2).
m(M) :- M = true ; M = false ; M = if(M1, M2, M3), m(M1), m(M2), m(M3) ; M = X, x(X) ; M = (fn(X : T1) -> M1), x(X), t(T1), m(M1) ; M = M1 $ M2, m(M1), m(M2) ; M = (let(X) = M1 in M2), x(X), m(M1), m(M2).
true![(J -> M)] subst true.
