...
Γ/St /- (fn(X <: _) => M11)![T2] ==> M11_/St              :- M11![(X -> T2)] tmsubst M11_.
Γ/St /- (let(_, X) = {(T11, V12)} as _ in M2) ==> M2__/St :- v(V12), subst(X, V12, M2_), M2_![(X -> T11)] tmsubst M2__.
...
Γ /- M1 $ M2 : bot                                :- !, Γ /- M1 : T1, lcst(Γ, T1, bot), Γ /- M2 : T2.
...
Γ /- case(M, Cases) : _                           :- writeln(error : typeof(Γ, case(M, Cases))), !, fail.
...
Γ /- (M1 := M2) : unit                            :- Γ /- M1 : T, lcst(Γ, T, sink(T1)), Γ /- M2 : T2, subtyping(Γ, T2, T1).
Γ /- loc(l) : _                                   :- !, fail.
Γ /- try(M1, M2) : T                              :- Γ /- M1 : T1, Γ /- M2 : T2, Γ /- T1 /\ T2 : T.
Γ /- error : bot                                  :- !.
Γ /- ({(T1, M2)} as T) : T                        :- Γ /- T :: '*', simplify(Γ, T, {some(Y :: TBound), T2}),
...
check_someBind(X:TBody, {(_, T12)} as _, X:TBody=T12).
check_someBind(X:TBody, _, X:TBody).

run({(TX, X)} = M, Γ/St, [B,TX<:K|Γ]/St_) :- tx(TX), x(X), m(M), !, Γ /- M : T, !,
