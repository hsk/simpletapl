...
k ::= '*' | kArr(k, k).
i ::= invariant | covariant.
t ::= bool | nat | unit | float | string | top | x | (t -> t) | {list(l : (i, t))} | (all(x :: t) => t) | (some(x :: t) => t) | abs(x, k, t) | t $ t.
m ::= true | false | if(m, m, m) | 0 | succ(m) | pred(m) | iszero(m) | unit | floatl | m * m | stringl | x | (fn(x : t) -> m) | m $ m | (let(x) = m in m) | fix(m) | inert(t) | m as t | {list(l = (i, m))} | update(m, l, m) | m # l | pack(t, m, t) | unpack(x, x, m, m) | (fn(x :: t) => m) | m![t].
n ::= 0 | succ(n).
v ::= true | false | n | unit | floatl | stringl | (fn(x : t) -> m) | {list(l = (i, v))} | pack(t, v, t) | (fn(x :: t) => v).
...
Γ \- {SF} <: {TF}                                    where maplist([L : (Vart, T)] >> (
                                                             member(L : (Vars, S), SF),
                                                             ((Vars = invariant ; Vart = covariant), Γ /- S <: T)
                                                           ), TF).
...
Γ /- update(M1, L, M2) : T1                       where Γ /- M1 : T1, Γ /- M2 : T2, lcst(Γ, T1, {Tf}),
                                                        member(L : (invariant, T), Tf), Γ /- T2 <: T.
...
run(someBind(TX, X, M), Γ, [X - B, TX - bTVar(TBound) | Γ])
                                       :- !, Γ /- M : T, lcst(Γ, T, (some(_ :: TBound) => TBody)), Γ /- M ==>> M_, check_someBind(TBody, M_, B),
